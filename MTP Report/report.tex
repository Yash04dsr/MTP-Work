...
\subsection{Pressure drop is not a differentiator}
\label{sec:cross4_dp}

After LP-filtering, the four campaign medians for
\(|\Delta p_{\mathrm{rgh}}|\) are \SI{1.05}{\kilo\pascal}
(\(90^\circ\)~top), \SI{0.53}{\kilo\pascal} (\(30^\circ\)~top),
\SI{1.58}{\kilo\pascal} (\(90^\circ\)~bot) and
\SI{1.02}{\kilo\pascal} (\(30^\circ\)~bot). All four lie within a
\SI{1}{\kilo\pascal} band --- well inside the per-case LP-filter
uncertainty of \(\pm\SI{0.5}{\kilo\pascal}\). Switching from top to
bottom injection therefore does not save pumping energy; it only
degrades mixing. This makes top injection \emph{strictly dominant}
over bottom injection in the present parameter space.
...
\begin{itemize}
    ...
    \(90^\circ\) top campaign; with the full angle\(\times\)side
    dataset now available, the same chain can be re-fit on
    all 40 cases and a final \((\dD,\VR,\alpha,\text{side})\)
