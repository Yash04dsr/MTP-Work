...
% Tight paragraph and list spacing to maximise page utilisation
% without crowding the eye.
\setlength{\parskip}{1pt plus 1pt minus 0pt}
\setlength{\parindent}{1.2em}
\usepackage{enumitem}
\setlist{topsep=1pt,itemsep=0.5pt,parsep=0pt,leftmargin=1.5em}
...
% - section headings (journal-ish, tightened spacing) -
...
\titlespacing*{\section}{0pt}{6pt plus 2pt minus 1pt}{3pt plus 1pt minus 0pt}
\titlespacing*{\subsection}{0pt}{4pt plus 2pt minus 1pt}{2pt plus 1pt minus 0pt}
...
\begin{titlepage}
    ...
    \vspace{0.25cm}
    {\Large \bfseries Prof.~Abhijit Raj \par}
...
with gravity are solved in OpenFOAM's
\texttt{rhoReacting\-Buoyant\-Foam} solver. Letting \(\rho\) be
the mixture density, \(\mathbf{u}\) the velocity,
\(p_{\mathrm{rgh}} = p - \rho\,\mathbf{g}\!\cdot\!\mathbf{x}\) the
modified pressure (so that the gravity term enters explicitly
...
\texttt{constant/}\allowbreak\texttt{thermophysicalProperties}),
this corresponds to
...
angle are \((\dD,\VR)\); the hydrogen blending ratio \(\HBR\) is
their derived consequence via a volumetric-flow balance,
\begin{equation}
\HBR \;=\;
...
\label{eq:hbr}
\end{equation}
so prescribing \((\dD,\VR)\) automatically fixes \HBR; \HBR{} is
reported per case for engineering readability but is not a third
independent design factor.

A 2-factor \((\dD,\VR)\) full factorial at five practical \(\dD\)
bins would already need \(5\times 5 = 25\) cases per angle; we
use a sliced Latin Hypercube design instead
...
(\HBR{} is the engineering-meaningful flow-rate variable and
bounding it is the cleanest way to span the realistic
...
opposite-wall jet impingement. This yields the 10-case design per
angle (\(5\times 2\)) given in Table~\ref{tab:doe_design}. Cases
that share a \(\dD\) also share a mesh, so the mesh is built once
per slice and re-used. The randomisation seed is fixed across
angles, so case~05 of the \(90^\circ\) campaign and case~05 of
the \(30^\circ\) campaign share all flow parameters
(\(\dD\) and \(\VR\), and therefore \HBR) and differ only in the
tilt; this is what makes the matched-pair analysis in
Section~\ref{sec:cross} possible.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../doe/cross_campaign/figures/fig_paired_comparison.png}
    \caption{Matched-pair angle effect. Each pair shares the
    independent \((\dD,\VR)\) (and therefore the derived \HBR);
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{../doe/cross_campaign/figures/fig_powerlaw_CoV_vs_VR.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{../doe/cross_campaign/figures/fig_pareto.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../doe/cross_campaign/figures/fig_heatmap_CoV.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{../doe/cross_campaign/figures/fig_loss_coefficient.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../doe/cross_campaign/figures/fig_xz_montage.png}
    \caption{Side-by-side \(Y_{\Hi}\) field on the \(x=0\) plane
    for matched \(\dD\) bins. Left column: \(\alpha=90^\circ\).
    Right column: \(\alpha=30^\circ\). Per-row title gives the
    case number and the corresponding \CoV. The tilted plume
    diffuses across the cross-section significantly earlier than
    the perpendicular plume at every matched bin.}
...
\begin{enumerate}
    ...
    \item For \(\alpha=90^\circ\), the nine completed production
    ...
    \item For \(\alpha=30^\circ\), the nine completed cases place
    case~01 at \(\CoV=0.005\) - an order of magnitude below the
    industrial target - and case~09 at the cheapest pumping cost
    in the entire dataset
    (\(|\Delta p|\!\approx\!\SI{0.3}{\kilo\pascal}\), within the
    LP-filter uncertainty band).
    The mesh-defect that initially invalidated this campaign was
    root-caused (an over-sized zipper rectangle in
    \texttt{generateSTL.py}) and fixed; the post-processing
    pipeline was extended with an angle-aware analytical mask and
    a non-orphan kNN reconstruction so that the figures match the
    \(90^\circ\) aesthetic.
    \item The matched-pair analysis on the seven shared LHS points
    shows that the \(30^\circ\) tilt reduces \CoV{} in
    \textbf{all seven} pairs (median improvement \(46\,\%\)) and
    reduces \(|\Delta p_{\mathrm{rgh}}|\) in \textbf{all seven}
    pairs (median improvement \(63\,\%\)). The power-law fit
    \(\CoV \sim \VR^{b}\) is steeper at \(30^\circ\) than at
    \(90^\circ\) (\(b=-1.88\) vs.\ \(-1.16\)), so each unit of
    \(\VR\) is converted into more mixing on the tilted geometry.
    The non-dimensional junction loss coefficient
    \(K = |\Delta p|/q_{\mathrm{dyn,main}}\) lands in the
    \([0.5,\,1.4]\) range at \(90^\circ\) and \([0.1,\,1.8]\) at
    \(30^\circ\), inside the unshaped diverging-tee envelope of
    \citet{idelchik1986handbook}.
    \item The combined Pareto front therefore shifts down-and-left
    with the tilt: every \(90^\circ\) operating point in this
    dataset is dominated by some \(30^\circ\) point. The two
    extreme designs at \(30^\circ\), case~01 (best mixing) and
    case~09 (cheapest pumping, within the
    \(\pm\SI{0.5}{\kilo\pascal}\) LP-filter band), bracket a
    broad operating window in which the design target is met
    without an in-line mixer.
\end{enumerate}

\subsection{Practical recommendation}
Where the layout permits a moderately tilted branch instead of a
perpendicular T, the data say it should be used. The tilt relaxes
both halves of the mixing-vs-pumping trade-off simultaneously, with
no change to the upstream or downstream piping other than the
branch angle. Within the present operating box, the recommended
designs are case~01 of the \(30^\circ\) campaign for tight mixing
budgets and case~06 of the \(30^\circ\) campaign for balanced
mixing and pumping cost; case~09 is the choice when pumping power
is the dominant constraint.

\subsection{Outlook}
Within the Minor-Project window the project has only two items
left, both already configured on the same case template and on
the newly-secured large-compute allocation, so they will be
folded into the dataset and the surrogate before the
end-semester presentation. Two further directions are flagged
below as longer-term extensions beyond the Minor-Project
window.

\paragraph{Remaining inside the Minor-Project window.}
\begin{itemize}
    \item \textbf{Closing the angle axis at the same
    fixed-(\(\dD,\VR\)) seed.}\;
    The DoE template, sliced LHS sampler and post-processing
    pipeline (built and frozen by the mid-semester review) are
    already configured for the remaining injection-angle
    campaign(s); the runs will be executed on the newly-secured
    compute and merged into the cross-campaign analysis to draw a
    smooth angle-vs-mixing curve. Case~04 of the \(30^\circ\)
    campaign, which reached only \(t=\SI{0.215}{\second}\) inside
    its original wall-time slot, is queued on the same compute
    allocation and will plug the matched-pair hole at
    \(\dD=0.252,\,\VR=3.81\) - the very operating point at which
    the \(90^\circ\) campaign is at its best, and therefore the
    most informative single addition.
    \item \textbf{Re-fitting the surrogate-MOO chain on the
    merged dataset.}\;
    The neural-network surrogate (regressor for \CoV{} and
    \(|\Delta p|\)) and the NSGA-II Pareto optimiser that sit
    downstream of the CFD dataset were prototyped on the
    \(90^\circ\) campaign at the mid-semester review; once the
    angle axis above is closed, the same chain will be re-fit on
    the merged \(90^\circ\)/\(30^\circ\)/(remaining-angle) data
    and a final \((\dD,\VR,\alpha)\) Pareto front reported at the
    end-semester presentation. No new modelling layer is required
    - only the additional CFD points to feed it.
\end{itemize}

\paragraph{Longer-term extensions beyond the Minor-Project window.}
\begin{itemize}
    ...
    helical insert with the tilt-best operating point would test
    whether a much more compact mixing length is achievable; this
    is the industrially relevant direction for any pipeline that
    cannot afford the \(\sim 5\,D\) of mixing run length implied
    by the no-mixer designs above.
    ...
    that, while well below the matched-pair angle-effect signal
    here, would matter for any future single-design fine-tuning.
    ...
    reduce the LP residual by an order of magnitude and let the
    junction loss coefficient be reported to two significant
...
simulations in this report - the parametric STL generator, the
\texttt{snappyHexMesh}/\texttt{blockMesh} templates, the
\texttt{rhoReactingBuoyantFoam} case template, the sliced-LHS
sampler, the per-case post-processing pipeline (\CoV{} and the
LP-filtered \(|\Delta p_{\mathrm{rgh}}|\)) and the cross-campaign
analysis script that produces every figure in
Section~\ref{sec:cross} - together with the raw OpenFOAM
\texttt{postProcessing} time-series for every completed case, is
open-source and version-controlled at
\url{https://github.com/Yash04dsr/MTP-Work}; a reviewer can
reproduce any single case via its \texttt{Allrun} script and the
cross-campaign figures via the analysis script in the
\texttt{doe/cross\_campaign/} directory.

% =====================================================================
%  ACKNOWLEDGEMENTS
% =====================================================================
\section*{Acknowledgements}
I thank Prof.~Abhijit Raj for the supervision and the latitude to
