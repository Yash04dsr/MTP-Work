...
# Operating constants (from case.env)
U_MAIN = 10.0       # m/s, main inlet velocity (constant across all cases)
RHO_MAIN_REF = 5.7  # kg/m^3, approx CH4 at 7 MPa, 300 K (q_dyn estimator only)
...
class CaseRow:
    ...
    dP_rgh_kPa: float    # signed, area-weighted
...
def load_30deg() -> list[CaseRow]:
    ...
    for r in rows:
        ...
        dP_kPa = _fnum(r.get("dP_p_rgh_area_kPa"))
        out.append(CaseRow(
            campaign="30deg",
...
def load_90deg() -> list[CaseRow]:
    """The 90 deg campaign uses a simpler summary (CoV_area, dP_p_rgh_area in
    Pa).  case_01 is missing data; skip it."""
    src = DIR_90 / "doe_summary" / "doe_summary_table.csv"
    rows = _read_csv_dicts(src)
    out = []
    for r in rows:
        cov_raw = (r.get("CoV_area") or "").strip()
        dP_raw  = (r.get("dP_p_rgh_area_Pa") or "").strip()
        if cov_raw in ("", "nan") or dP_raw in ("", "nan"):
            continue
        cov = _fnum(cov_raw)
        dP_Pa = _fnum(dP_raw)
        if not (math.isfinite(cov) and math.isfinite(dP_Pa)):
            continue
        ...
        out.append(CaseRow(
            ...
            Re_branch=float("nan"),
            CoV=cov,
            dP_rgh_kPa=dP_Pa / 1000.0,
            abs_dP_kPa=abs(dP_Pa) / 1000.0,
        ))
    return out
