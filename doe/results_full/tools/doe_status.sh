...
elapsed_sec() {
    # Use birth time (%W) if available; fall back to ctime via the case
    # directory (less drift than log.solver's mtime which keeps bumping).
    local log="$1"
    [[ -f "$log" ]] || { echo "-"; return; }
    local start
    start=$(stat -c %W "$log" 2>/dev/null)
    if [[ -z "$start" || "$start" == "-" || "$start" == "0" ]]; then
        # Try ctime of the parent case dir (created on stamp).
        start=$(stat -c %Z "$(dirname "$log")" 2>/dev/null)
    fi
    local now=$(date +%s)
    echo $(( now - start ))
}
...
for d in "${CASES[@]}"; do
    ...
    if [[ -f "$resdir/SIM_DONE" || -f "$d/SIM_DONE" ]]; then
        ...
        if tac "$logS" 2>/dev/null | head -n 20 | grep -qE '^End$'; then
