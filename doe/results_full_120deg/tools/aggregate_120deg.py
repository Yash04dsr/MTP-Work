...
Aggregates per-case metrics from `all_metrics.csv` files of the 120 deg DoE
...
def pareto_dP_vs_CoV(rows, *, out):
    ...
    ax.set_title("120 deg: Pareto frontier - pressure-drop vs. mixing")
    ax.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(out, bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    write_csv(rows, args.out / "doe_summary_120deg.csv")
    write_md(rows,  args.out / "DOE_SUMMARY_120DEG.md")

    print("Plotting scatter pack ...")
    scatter_xy(rows, "d_over_D", "CoV_mass_tavg", "HBR",
               xlabel="d/D", ylabel="CoV (mass-flux, time-avg H$_2$)",
               clabel="HBR", title="120 deg: CoV vs. d/D",
    ...
    scatter_xy(rows, "VR", "CoV_mass_tavg", "HBR",
               ...
               clabel="HBR", title="120 deg: CoV vs. VR",
    ...
    scatter_xy(rows, "HBR", "CoV_mass_tavg", "d_over_D",
               ...
               clabel="d/D", title="120 deg: CoV vs. HBR",
