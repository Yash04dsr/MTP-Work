...
def run(case_dir: str, times: list[str] | None, outdir: str | None) -> None:
    ...
    # ---- Write CSV ----
    # Build the full key set across snapshot rows and the AVG row, so that
    # time-averaged-only keys (H2_*_tavg, dP_*_ts, ...) survive into the CSV.
    extra_keys = [k for k in avg.keys() if k not in keys]
