...
|---|---:|---:|---:|---:|
| area | 0.01420 | 0.01187 | 0.8353 | 0.0101 |
| mass | 0.01236 | 0.01140 | 0.9224 | 0.0106 |
| vol  | 0.01319 | 0.01154 | 0.8746 | 0.0102 |
...
- ⟨Y_H₂⟩_area, time-series: **0.00518** (n = 56 samples in [0.804, 1.197] s)

## 2. Pressure drop

### 2a. Clean reference from function-object time series (area-weighted, every timestep)

- ΔP_area_ts on `p_rgh`: **-0.920 kPa**  (⟨p_rgh⟩_inlet = 6899.1 kPa, ⟨p_rgh⟩_outlet = 6900.0 kPa, n = 56 samples)
...
|---|---:|---:|---:|
| static `p` | 18.885 | 18.886 | 18.886 |
| gauge `p_rgh` | 18.882 | 18.875 | 18.878 |
| total `p + ½ρU²` | 19.424 | 19.396 | 19.416 |
| gauge-total `p_rgh + ½ρU²` | 19.420 | 19.386 | 19.409 |

## 3. Mass balance

### 3a. Clean reference from outletFlux function object (every timestep)

- ⟨sum(phi)⟩_outlet (ṁ_out, kg/s), time-averaged: **+36.9708**  (σ = 8.5955, n = 56 samples, t ∈ [0.804, 1.197] s)

### 3b. From 3 snapshots (instantaneous, dominated by acoustic pulses)

- ṁ main_inlet  : **-33.7385** kg/s
- ṁ branch_inlet: **-0.5350** kg/s
- ṁ outlet      : **+26.9272** kg/s
- closure error : **-7.3463e+00 kg/s**  (-27.28 % of ṁ_outlet)

## 4. Per-snapshot detail

| time | CoV_area | CoV_mass | CoV_vol | ΔP_static_mass [kPa] | ΔP_total_mass [kPa] | balance [%] |
|---|---:|---:|---:|---:|---:|---:|
| 1.2 | 0.8353 | 0.9224 | 0.8746 | 18.886 | 19.396 | -27.28 |
| **AVG** | **0.8353** | **0.9224** | **0.8746** | **18.886** | **19.396** | **-27.28** |
