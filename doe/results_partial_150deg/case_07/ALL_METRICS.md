...
|---|---:|---:|---:|---:|
| area | 0.02093 | 0.00365 | 0.1743 | 0.0006 |
| mass | 0.02098 | 0.00360 | 0.1715 | 0.0006 |
| vol  | 0.02106 | 0.00359 | 0.1707 | 0.0006 |
...
- ⟨Y_H₂⟩_area, time-series: **0.01672** (n = 236 samples in [0.800, 1.199] s)

## 2. Pressure drop

### 2a. Clean reference from function-object time series (area-weighted, every timestep)

- ΔP_area_ts on `p_rgh`: **3.689 kPa**  (⟨p_rgh⟩_inlet = 6903.7 kPa, ⟨p_rgh⟩_outlet = 6900.0 kPa, n = 236 samples)
...
|---|---:|---:|---:|
| static `p` | -5.933 | -5.932 | -5.932 |
| gauge `p_rgh` | -5.933 | -5.932 | -5.932 |
| total `p + ½ρU²` | -6.704 | -6.666 | -6.668 |
| gauge-total `p_rgh + ½ρU²` | -6.705 | -6.666 | -6.668 |

## 3. Mass balance

### 3a. Clean reference from outletFlux function object (every timestep)

- ⟨sum(phi)⟩_outlet (ṁ_out, kg/s), time-averaged: **+35.5410**  (σ = 4.5362, n = 236 samples, t ∈ [0.800, 1.199] s)

### 3b. From 3 snapshots (instantaneous, dominated by acoustic pulses)

- ṁ main_inlet  : **-33.6192** kg/s
- ṁ branch_inlet: **-0.7030** kg/s
- ṁ outlet      : **+37.7671** kg/s
- closure error : **+3.4449e+00 kg/s**  (+9.12 % of ṁ_outlet)

## 4. Per-snapshot detail

| time | CoV_area | CoV_mass | CoV_vol | ΔP_static_mass [kPa] | ΔP_total_mass [kPa] | balance [%] |
|---|---:|---:|---:|---:|---:|---:|
| 1.2 | 0.1743 | 0.1715 | 0.1707 | -5.932 | -6.666 | +9.12 |
| **AVG** | **0.1743** | **0.1715** | **0.1707** | **-5.932** | **-6.666** | **+9.12** |
